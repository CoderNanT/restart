...
- 一个网页URL从输入到浏览器中，到显示经历过怎么样的解析过程呢？
  - **URL 输入**
    ...
    - 判断输入的 URL 是否完整，如果不完整，浏览器可能会对域名进行猜测，补全前缀或者后缀
    - 使用用户设置的默认搜索引擎来进行搜索
  - **DNS 解析**
  ...
  - **建立 TCP 连接**
  - <b>发送 HTTP / HTTPS 请求（建立 TLS 连接）</b>
    - 向服务器 发起 TCP 连接请求
    - 当这个请求到达服务端后，通过 TCP 三次握手，建立 TCP 的连接
      - 客户端发送 SYN 包到服务器，并进入SYN SEND 状态，等待服务器确认
      - 服务器收到 SYN 包，必须确认客户端的 SYN，同时自己也发送一个 SYN 包，此时服务器进入 SYN RECV 状态
  ...
  - **浏览器解析渲染页面**
    - 加载HTML、 解析HTML，生成DOM树
    - 加载CSS、解析CSS，生成样式规则
    - 将 DOM 与 样式规则 合并成一个渲染树
    - 根据渲染树来布局，以计算每个节点的宽高、位置
    - 将各个节点绘制到屏幕上
...
- 更详细的解析过程如下
  ...
  3. 将 DOM 与 样式规则 合并成一个渲染树
  4. 根据渲染树来布局，以计算每个节点的宽高、位置
  5. 将各个节点绘制到屏幕上
  6. 显示
...
- 第四步是在渲染树（Render Tree）上运行<b>布局（Layout）</b>以计算每个节点的宽高、位置
  - 渲染树会表示显示哪些节点以及其他样式，但是**不会计算每个节点的宽高、位置**等信息
...
- 什么情况下引起回流呢？
  ...
  - 比如窗口resize（修改了窗口的宽高等）
  - 比如调用getComputedStyle方法获取宽高、位置信息
...
- 所以在开发中要尽量避免发生回流
  ...
  - 尽量**避免通过getComputedStyle获取宽高、位置**等信息
...
- js引擎会在**执行代码之前**，会在**堆内存中创建一个全局对象 Global Object（GO）**
  ...
  - 用于存放一些**定义好的变量方法**等，包含**Date、Array、String、Number、setTimeout、setInterval**等等
  - 同时有一个**window属性**指向自己
  - 该对象**所有的作用域**都可以访问
  
  - 同时在语法分析转成 AST 的过程中也会将一些**变量、函数**存放在 GO 中只是变量的初始值为 **undefifined**
...
- 因为每个执行上下文都会关联一个VO，那么函数执行上下文关联的VO是什么呢？
  ...
  - 当进入一个函数执行上下文时（函数在执行前），会先在堆内存中创建一个**AO（Activation Object）对象**，里面存放着 arguments 对应函数的形参，以及在函数中定义的变量，初始值为 undefifined
...
- **作用域**是指**变量**和**函数**的**可访问范围**
  ...
  - 在 ES5 中，全局是⼀个作用域，函数也会产生自己的作用域
  - 在 ES6 中，代码块、let、const 等都会有自己的作用域
...
- 当然上面的定义会过于的晦涩，所以我简单总结一下
  ...
  - **确定的输入，一定会产生确定的输出**（外部环境的任何变化不会影响函数内部的操作所产生的结果）
  - **函数在执行过程中，不能产生副作用**（函数内部的操作也不会对函数外部产生任何影响）
...
- **无法意外的创建全局变量（不写var、let、const这种声明变量的关键字）**

- **严格模式会引起静默错误的赋值操作抛出异常（silently fail，注：不报错也没有任何效果）**
...
- 我们前面讲过new关键字的步骤如下
  ...
  - 在内存中创建一个新的对象（空对象）
  - 将**构造函数**的**显式原型（prototype）**赋值给新对象的**隐式原型（[[prototype]]）**
    * `obj.__proto__ = function.prototype `
